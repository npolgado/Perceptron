...
class Perceptron:
    def __init__(self, learning_rate=0.1, weights=None, bias=20):
        """
    ...
    def save_weights(self):
        ...
        fig_path = os.path.join(visuals_folder, f"{self.adjusted}_{date}.png")
        ...
        if self.save_path:
            if os.path.exists(self.save_path):
                ...
                fig_path = os.path.join(visuals_folder, f"{self.adjusted}_{weights_file_name}.png")
...
try:
    p = Perceptron(0.1, np.zeros((RESIZE_FACTOR, RESIZE_FACTOR)))
    st = time.monotonic()
    is_face = False
    root = os.path.dirname(__file__)
